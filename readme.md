...
See [Full Documentation](https://github.com/falconsw/elasticsearch/wiki/1.-Installation).
...
```bash
$ composer require falconsw/elasticsearch
```

##### 2) Add package service provider (< laravel 5.5).

```php
Falconsw\Elasticsearch\ElasticsearchServiceProvider::class
```

##### 3) Add package alias (< laravel 5.5).

```php
'ES' => Falconsw\Elasticsearch\Facades\ES::class
```
	
##### 4) Publishing.

```bash
$ php artisan vendor:publish --provider="Falconsw\Elasticsearch\ElasticsearchServiceProvider"
```
...
```php
$app->register(Falconsw\Elasticsearch\ElasticsearchServiceProvider::class);
```
	
##### 3) Copy package config directory `vendor/falconsw/elasticsearch/src/config` to root folder alongside with `app` directory.
...
use Falconsw\Elasticsearch\Connection;
...
use Falconsw\Elasticsearch\Model;
...
class Post extends Model
{
     ...
     *
     * @param \Falconsw\Elasticsearch\Query $query
     * @return \Falconsw\Elasticsearch\Query
     */
